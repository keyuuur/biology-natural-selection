...
- A deliberately QA-enabled preview may expose a closed, in-flow facilitator diagnostics panel only when its URL includes `?qa=1`. It is never part of the ordinary student route and never mounts in a normal production build. It reports only resettable in-memory aggregate input outcomes, input-handler-to-feedback callback latency, and renderer lifecycle/performance facts; it stores, sends, and renders no student identity, answers, CER text, or raw organism details. The panel supports Gate 1 technical observation but does not substitute for visible iPad feedback, Safari, Wi-Fi, VoiceOver, or student-pilot evidence.
- `VITE_INTERACTION_QA=1` is deliberately narrower than the local E2E harness: a facilitator preview may show the aggregate panel at `?qa=1`, but it must ignore every `?e2e=1` timer, seed, renderer, and storage override and must not publish the raw interaction bridge. E2E overrides and raw actor diagnostics are limited to local development or an explicitly non-public `VITE_E2E_CONTROLS=1` artifact.
- Resetting Facilitator diagnostics begins a new device-measurement window: aggregate outcomes, callback-latency samples, frame samples, and duplicate-round-end count reset together. It must never change a population, timer, HUD, actor, assessment response, or draft.
- On shared devices, beginning a new study, choosing Start over from draft recovery, or replaying after Results clears both the local draft and the prior locally saved result. This removes a previous student's free-text CER before the next study begins; storage failure never blocks a new study.
- Draft recovery is a focus-managed modal: Resume receives initial focus, background controls are inert/hidden from assistive technology while it is open, Tab wraps within its two actions, and closing it returns focus to the visible current stage heading.
